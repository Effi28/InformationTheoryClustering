 ...
 *    Copyright (C) 2010-2002 Luigi Dragone
 *
 */
...
public class SpectralClusterer extends AbstractClusterer {
	...
	protected final static Collection<Option> OPTIONS = Arrays.asList(
			...
			new Option(
					...
					"R", 1, "-R <num>"),
			new Option("\tUse sparse matrix representation. (default = false).",
					"M", 0, "-M"),
			new Option(
					"\tDistance function to use.\n"
							+ "\t(default: weka.core.EuclideanDistance)",
					"D", 1, "-D <classname and options>"));
	 ...
	 * @param a the points of the first partition
	 * @param b the points of the second partition
	 * @param c the points of the subgraph
	 * @return the normalized association degree
	 */
	 ...
	 */
	protected double Ncut(final DoubleMatrix2D W, final int[] a,
			final int[] b) {
	 ...
	 * 
	 * @param W the weight matrix of the graph
	 * @param alphaStar the alpha star factor
	 * @return an array of sets of points (partitions)
	 */
	protected int[][] partition(final DoubleMatrix2D W) {
	...
	public int clusterInstance(final Instance instance) {
		...
		for (int i = 0; i < getData().numInstances(); i++) {
			final double dist = getDistanceFunction().distance(instance,
					getData().instance(i));
			if (dist < min_dist) {
				c = cluster[i];
				min_dist = dist;
	...
	public void buildClusterer(final Instances data) throws Exception {
		...
		DoubleMatrix2D w = useSparseMatrix ? DoubleFactory2D.sparse.make(n, n)
		...
		for (int i = 0; i < n; i++)
			for (int j = i; j < n; j++) {
				final double dist = getDistanceFunction()
						.distance(getData().instance(i), getData().instance(j));
				if ((r <= 0) || (dist < r)) {
		...
		// w = test.calculateGraph(Parameters.ALGORITHM);
		// }
		// Compute point partitions
		final int[][] p = partition(w);
	...
	public void setOptions(final String[] options) throws Exception {
		...
		if (optionString.length() != 0) {
			final String distFunctionClassSpec[] = Utils
					.splitOptions(optionString);
	...
	public String[] getOptions() {
		...
		options.add((getDistanceFunction().getClass().getName() + " "
				+ Utils.joinOptions(getDistanceFunction().getOptions()))
						.trim());
